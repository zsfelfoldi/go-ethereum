...
type Filter struct {
	...
	//rangeLogsTestHook chan rangeLogsTestEvent  //TODO ???
}
...
func (f *Filter) rangeLogs(ctx context.Context, firstBlock, lastBlock uint64) ([]*types.Log, error) {
	...
	chainView := f.sys.backend.ChainView()
	indexView := f.sys.backend.IndexView()
	...
	indexedRange := indexView.SharedRange(chainView).Intersection(searchRange)
	...
	res2, err = f.indexedLogs(ctx, indexView, indexedRange.First(), indexedRange.Last())
	indexView.Release()
	if err != nil {
		return nil, err
	}
...
func (f *Filter) indexedLogs(ctx context.Context, indexView *filterMaps.IndexView, begin, end uint64) ([]*types.Log, error) {
	start := time.Now()
