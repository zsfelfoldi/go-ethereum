...
var (
	...
	balanceField = utils.NewNodeStateField("balance", reflect.TypeOf(&NodeBalance{}), nil, false, nil, nil)
...
func NewBalanceTracker(ns *utils.NodeStateMachine, db ethdb.KeyValueStore, clock mclock.Clock, posExp, negExp utils.ValueExpirer, nodeField *utils.NodeStateField) *BalanceTracker {
	...
	ns.AddFieldSub(bt.nodeFieldIndex, func(id enode.ID, state utils.NodeStateBitMask, oldValue, newValue interface{}) {
		if newValue != nil {
			ns.SetField(id, bt.balanceFieldIndex, bt.newNodeBalance(id, newValue.(btNode).FreeID()))
		} else {
			n := bt.GetNode(id)
			n.deactivate(true)
			ns.UpdateState(id, 0, bt.stPriority, 0)
			ns.SetField(id, bt.balanceFieldIndex, nil)
		}
	})
	...
	bt.ndb.evictCallBack = func(now mclock.AbsTime, neg bool, b utils.ExpiredValue) bool {
		...
		return b.Value(expiration) <= uint64(time.Second)
	}
...
// stop shuts down the balance tracker and prevents any further changes
func (n *NodeBalance) deactivate(close bool) {
	...
	n.bt.storeBalance(n.id.Bytes(), false, n.balance.pos, &n.storedBalance.pos, n.bt.posExp)
	n.bt.storeBalance([]byte(n.freeID), true, n.balance.neg, &n.storedBalance.neg, n.bt.negExp)
	n.closed = close
}
...
func (n *NodeBalance) balanceExhausted() {
	...
	n.bt.storeBalance(n.id.Bytes(), false, n.balance.pos, &n.storedBalance.pos, n.bt.posExp)
	n.priority = false
	n.bt.ns.UpdateState(n.id, 0, n.bt.stPriority, 0)
}

func (bt *BalanceTracker) storeBalance(id []byte, neg bool, value utils.ExpiredValue, storedValue *utils.ExpiredValue, exp utils.ValueExpirer) {
...
func (bt *BalanceTracker) TotalTokenAmount() uint64 {
	...
	if now > bt.lastActiveBalanceUpdate+mclock.AbsTime(time.Second) {
		bt.ns.ForEach(bt.stActive, 0, func(id enode.ID, state utils.NodeStateBitMask) {
...
func (n *NodeBalance) requestCost(cost uint64) uint64 {
	...
	if fcost > 0 {
		if n.negFactor.reqPrice() != 0 {
			n.balance.neg.Add(int64(fcost*n.negFactor.reqPrice()), n.bt.negExp.LogOffset(now))
			n.checkCallbacks(now)
		}
	}
	n.sumReqCost += cost
