...
type FilterQuery struct {
	Pattern     PatternMatcher `json:"pattern"`
	FirstBlock  uint64         `json:"firstBlock"`
	LastBlock   uint64         `json:"lastBlock"`
	Limit       uint64         `json:"limit"`
	LimitLatest bool           `json:"limitLatest"`
}
...
func (mc *Matcher) GetMatches(ctx context.Context, chainView chainView, query FilterQuery, prove bool) ([]*types.Log, common.Range[uint64], *QueryProof, error) {
	...
	limitedQuery := query
	limitedQuery.Limit = uint64(rand.Intn(100) + 1)
	limitedQuery.LimitLatest = rand.Intn(2) == 1
...
func (mc *Matcher) getMatches(ctx context.Context, chainView chainView, query FilterQuery, prove bool) ([]*types.Log, common.Range[uint64], *QueryProof, error) {
	...
	if uint64(maxResults) > query.Limit {
		maxResults = int(query.Limit)
	}
	session := newSession(ctx, mc, refBlockHash, maxResults, query.LimitLatest)
	...
	for i, tr := range readers {
		...
		matcherInstance := query.Pattern.newInstance(
			ctx,
			&directionalReader{reader: tr, reverse: query.LimitLatest},
			logicBuilder,
			firstPos,
			lastPos,
		)
	...
	fmt.Println("query", query, "results.firstBlock", results.firstBlock, "results.lastBlock", results.lastBlock)
...
func newSession(ctx context.Context, mc *Matcher, refBlockHash common.Hash, maxResults int, reverse bool) *matcherSession {
	ms := &matcherSession{
		ctx:          ctx,
...
func (mp *matcherProcess) newStatus(cumulativeResults uint64) int {
	...
	if cumulativeResults+uint64(len(mp.allMatches)) >= uint64(mp.session.maxResults) {
		if mp.prev == nil || mp.prev.status == mpFinishedAll {
...
func (ms *matcherSession) print() {
	for mp := ms.first; mp != nil; mp = mp.next {
		...
		fmt.Println(" range", mp.firstBlock, mp.lastBlock, "len(pos)", len(mp.positions), "len(allMatches)", len(mp.allMatches),
...
func (ms *matcherSession) returnResults() {
	...
	addProcessResults := func(mp *matcherProcess) bool {
		...
		for i, log := range mp.allMatches {
			...
			if log == nil {
				fmt.Println(mp.firstBlock, mp.lastBlock, "*** nil log at", i, "mp.finished", mp.finished, "len(mp.allMatches)", len(mp.allMatches))
			}
