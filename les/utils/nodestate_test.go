...
func registerTestFields(ns *NodeStateMachine, n int, flags []*NodeStateFlag) []*NodeStateField {
	...
	for i := 0; i < n; i++ {
		...
		field := NewNodeStateField(fmt.Sprintf("field-%d", i), reflect.TypeOf(enr.Record{}), []*NodeStateFlag{f})
...
func TestSaveImmediately(t *testing.T) {
	...
	fd, _ := ns.RegisterField(NewNodeStateField("field", reflect.TypeOf(""), []*NodeStateFlag{flag0, flag1, flag2}))
...
func TestRegistrationProtection(t *testing.T) {
	...
	var cases2 = []struct {
		...
		{fields[0], 0, nil},
		{fields[29], 29, nil},
		{NewNodeStateField("field-0", reflect.TypeOf(enr.Record{}), nil), 0, errNameCollision},
		{NewNodeStateField("field-29", reflect.TypeOf(enr.Record{}), nil), 0, errNameCollision},
		{NewNodeStateField("field-30", reflect.TypeOf(enr.Record{}), nil), 30, nil},
	}
	...
	if err != errAlreadyInited {
		t.Fatalf("Expect already init error")
	}
	_, err = ns.RegisterField(NewNodeStateField("field-31", reflect.TypeOf(enr.Record{}), nil))
	if err != errAlreadyInited {
		t.Fatalf("Expect already init error")
	}
...
func TestSetField(t *testing.T) {
	...
	fd, _ := ns.RegisterField(NewNodeStateField("field", reflect.TypeOf(""), []*NodeStateFlag{flag}))
...
func TestPersistent(t *testing.T) {
	...
	fd0, _ := ns.RegisterField(NewNodeStateField("field0", reflect.TypeOf(uint64(0)), []*NodeStateFlag{f0}))
	fd1, _ := ns.RegisterField(NewNodeStateField("field1", reflect.TypeOf(""), []*NodeStateFlag{f1}))
	ns.UpdateState(enode.ID{0x01}, s0, 0, time.Second)
	ns.UpdateState(enode.ID{0x01}, s1, 0, time.Second)
	ns.SetField(enode.ID{0x01}, fd0, 100)
	ns.SetField(enode.ID{0x01}, fd1, "hello world")
	ns.SaveToDb()

	ns2 := NewNodeStateMachine(mdb, []byte("-ns"), clock)
	s0, _ = ns2.RegisterState(f0)
	s1, _ = ns2.RegisterState(f1)
	fd0, _ = ns2.RegisterField(NewNodeStateField("field0", reflect.TypeOf(uint64(0)), []*NodeStateFlag{f0}))
	fd1, _ = ns2.RegisterField(NewNodeStateField("field1", reflect.TypeOf(""), []*NodeStateFlag{f1}))
	ns2.LoadFromDb()
	field0 := ns2.GetField(enode.ID{0x01}, fd0)
	if reflect.DeepEqual(field0, uint64(0)) {
	...
	// Different order
	s1, _ = ns3.RegisterState(f1)
	s0, _ = ns3.RegisterState(f0)
	fd1, _ = ns3.RegisterField(NewNodeStateField("field1", reflect.TypeOf(""), []*NodeStateFlag{f1}))
	fd0, _ = ns3.RegisterField(NewNodeStateField("field0", reflect.TypeOf(uint64(0)), []*NodeStateFlag{f0}))
	// additional registeration
	ns3.RegisterState(NewNodeStateFlag("flag2", false, true))
	ns3.RegisterField(NewNodeStateField("field2", reflect.TypeOf(uint32(0)), []*NodeStateFlag{f0}))
