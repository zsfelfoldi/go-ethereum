...
func (ns *NodeStateMachine) loadFromDb() {
	...
	for index, fieldInfo := range ns.nodeFields {
		mapping.Fields[index] = fieldInfo.field.name
	}
	ns.currentMapping = -1
loop:
...
func (ns *NodeStateMachine) decodeNode(id enode.ID, data []byte, now mclock.AbsTime) {
	...
	for i, encField := range enc.Fields {
		...
		node.fields[index] = reflect.New(ns.nodeFields[index].field.ftype).Interface()
	...
	return func() {
		...
		if newState != 0 {
			...
			if node.fieldGcCounter == 0 {
				...
				for i, f := range node.fields {
					if f != nil {
						if ns.nodeFields[i].mask&node.state == 0 {
							node.fields[i] = nil
						}
					}
				}
...
func (ns *NodeStateMachine) RegisterField(field *NodeStateField) (int, error) {
	...
	ns.nodeFields = append(ns.nodeFields, &nodeFieldInfo{
		field: field,
		mask:  mask,
	})
...
func (ns *NodeStateMachine) SetField(id enode.ID, fieldId int, value interface{}) error {
	ns.lock.Lock()
	cb, err := ns.setField(id, fieldId, value)
	ns.lock.Unlock()
	if cb != nil {
		cb()
	}
...
func (ns *NodeStateMachine) setField(id enode.ID, fieldId int, value interface{}) (func(), error) {
	ns.checkStarted()
	...
	if reflect.TypeOf(value) != f.field.ftype {
		log.Error("Invalid field type", "type", reflect.TypeOf(value), "required", ns.nodeFields[fieldId].field.ftype)
		return nil, errInvalidField
	}
	oldValue := node.fields[fieldId]
	state := node.state
	if f.mask&state == 0 || value == oldValue {
		return nil, nil
	}
	...
	if f.mask&ns.saveImmediately != 0 {
		err := ns.saveNode(id, node)
		if err != nil {
			log.Error("Failed to save node", "id", id, "error", err)
		}
	} else if f.mask&ns.saveAtShutdown != 0 {
		node.dirty = true
	}
	if len(f.subs) > 0 {
		return func() {
...
func (ns *NodeStateMachine) ForEach(mask NodeStateBitMask, cb func(id enode.ID, state NodeStateBitMask)) {
	ns.lock.Lock()
	type call struct {
		id    enode.ID
		state NodeStateBitMask
	...
	for id, node := range ns.nodes {
		if node.state&mask != 0 {
			calls = append(calls, call{id, node.state & mask})
		}
	}
	ns.lock.Unlock()
	for _, call := range calls {
		cb(call.id, call.state)
	}
}
