...
func ExecuteStateless(ctx context.Context, config *params.ChainConfig, vmconfig vm.Config, block *types.Block, witness *stateless.Witness) (common.Hash, common.Hash, error) {
	...
	validator := NewBlockValidator(config, nil, nil) //TODO // No chain, we only validate the state, not the block

	// Run the stateless blocks processing and self-validate certain fields
	res, err := processor.Process(ctx, block, db, nil /*TODO*/, nil, nil, vmconfig, nil)
	if err != nil {
		return common.Hash{}, common.Hash{}, err
	}
