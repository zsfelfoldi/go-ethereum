...
func (self *BlockRequest) Request(peer *odrPeer) error {
	glog.V(logger.Debug).Infof("ODR: requesting body of block %08x from peer %v", self.Hash[:4], peer.Id())
	return peer.GetBlockBodies([]common.Hash{self.Hash})
}
...
// Request sends an ODR request to the LES network (implementation of access.ObjectAccess)
func (self *ReceiptsRequest) Request(peer *odrPeer) error {
	glog.V(logger.Debug).Infof("ODR: requesting receipts for block %08x from peer %v", self.Hash[:4], peer.Id())
	return peer.GetReceipts([]common.Hash{self.Hash})
}
...
type NodeDataRequest light.NodeDataRequest

func (self *NodeDataRequest) Request(peer *odrPeer) error {
...
func (self *NodeDataRequest) Valid(db ethdb.Database, msg *Msg) bool {
	glog.V(logger.Debug).Infof("ODR: validating node data for hash %08x", self.Hash[:4])
	if msg.MsgType != MsgNodeData {
		glog.V(logger.Debug).Infof("ODR: invalid message type")
		return false
	}
	reply := msg.Obj.([][]byte)
