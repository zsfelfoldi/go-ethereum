...
type QueryProof struct {
	...
	RefHeader          types.Header      `json:"refHeader"`
	IndexContracts     []common.Address  `json:"indexContracts"`
	ContractProofNodes []hexutil.Bytes   `json:"contractProofNodes"`
	ContractProofCodes []hexutil.Bytes   `json:"contractProofCodes"`
	TableQueryProofs   []tableQueryProof `json:"tableQueryProofs"`
...
type tableQueryProof struct {
	...
	IndexContract uint32                `json:"indexContract"`
	EntryIndices  []uint64              `json:"entryIndices"`
	ProvenEntries logindex.IndexEntries `json:"provenEntries"`
	ProofHashes   []merkle.Value        `json:"proofHashes"`
	ResultCount   uint64                `json:"resultCount"`
	BlockResults  []blockResults        `json:"blockResults"`
}

type blockResults struct {
	Header         types.Header    `json:"header"`
	ProvenReceipts []uint32        `json:"provenReceipts"`
	ReceiptsProof  []hexutil.Bytes `json:"receiptsProof"` // receipts trie nodes in ascending order of node hash
...
func (qp *QueryProof) Verify(backend contractVerifierBackend) ([]*types.Log, error) {
	...
	if (qp.TableQueryProofs[0].FirstBlock > qp.Query.FirstBlock && (resultCount < qp.Query.Limit || !qp.Query.LimitLatest)) ||
...
// if total result count equals Limit then resultsLimited is true for the
...
func (tqp *tableQueryProof) verify(query *FilterQuery, provenTableRoot common.Hash, resultsLimited bool, results []*types.Log, reqParentBlockHash common.Hash) ([]*types.Log, common.Hash, error) {
	...
	results, positions, reqLastBlockHash, err := tqp.getProvenEntries(query, results, reqParentBlockHash)
	...
	//fmt.Println("getProvenEntries oldCount:", oldCount, "results:", len(results), "positions", len(positions))
	if err != nil {
		return nil, common.Hash{}, err
	}
	...
	if uint64(count) != tqp.ResultCount {
		//fmt.Println("count", count, "tqp.ResultCount", tqp.ResultCount, "len(positions)", len(positions))
		return nil, common.Hash{}, errors.New("invalid result count")
	}
	//fmt.Println(" * result count", count)
	potentialMatches := query.Pattern.proofMatches(tqp, begin, end)
	//fmt.Println(" * inclusion proven", positions)
...
// the count limit.
// Also note that the positions position list might be longer than the
// number of added results in case a log satisfies the matchSpecified but not
// the matchLength criteria.
func (tqp *tableQueryProof) getProvenEntries(query *FilterQuery, results []*types.Log, reqParentBlockHash common.Hash) ([]*types.Log, []logindex.IndexPosition, common.Hash, error) {
...
loop:
	for _, entry := range tqp.ProvenEntries {
	...
	if reqParentBlockHash != (common.Hash{}) {
		...
		if pbh, ok := provenBlockEntries[tqp.FirstBlock-1]; !ok {
			return nil, nil, common.Hash{}, errors.New("required proven parent block hash is missing")
		} else if pbh != reqParentBlockHash {
			return nil, nil, common.Hash{}, errors.New("required proven parent block hash does not match")
		}
	}
	...
	for i, br := range tqp.BlockResults {
		...
		if number < tqp.FirstBlock || number >= tqp.FirstBlock+tqp.TableSize || number < query.FirstBlock || number > query.LastBlock {
			return nil, nil, common.Hash{}, errors.New("invalid block proof")
		}
		if i > 0 && number <= lastNumber {
			return nil, nil, common.Hash{}, errors.New("invalid block proof order")
		}
		lastNumber = number
		blockHash := br.Header.Hash()
		if number < tqp.FirstBlock+tqp.TableSize-1 {
			if hash, ok := provenBlockEntries[number]; !ok {
				return nil, nil, common.Hash{}, errors.New("block entry missing")
			} else if hash != blockHash {
				return nil, nil, common.Hash{}, errors.New("block hash mismatch")
			}
		} else {
		...
		for _, txIndex := range br.ProvenReceipts {
			...
			if !ok {
				return nil, nil, common.Hash{}, errors.New("transaction entry missing")
			}
			...
			for i, log := range receipt.Logs {
				if query.Pattern.matchLogs(receipt.Logs, i) {
					positions = append(positions, logindex.IndexPosition{BlockNumber: number, TxIndex: txIndex, LogIndex: uint32(i)})
					rlog := &types.Log{
						Address:        log.Address,
						Topics:         log.Topics,
						Data:           log.Data,
	...
	return results, positions, reqLastBlockHash, nil
}
...
func (tqp *tableQueryProof) calculateTableRoot() (merkle.Value, error) {
	...
	for i, entryIndex := range tqp.EntryIndices {
		...
		hashes[leafOffset+entryIndex] = tqp.ProvenEntries[i].Hash()
...
func (tqp *tableQueryProof) getValueMatches(value logindex.IndexValue, begin, end logindex.IndexPosition) indexPositionSet {
	...
	/*for i, idx := range tqp.EntryIndices {
		fmt.Println(" ", idx, tqp.ProvenEntries[i])
	}*/
	boundary := logindex.IndexEntry{
		IndexValue:    value,
		IndexPosition: begin,
	}
	firstInRange, _ := tqp.ProvenEntries.Find(&boundary)
	boundary.IndexPosition = end
	afterLastInRange, lastFound := tqp.ProvenEntries.Find(&boundary)
	if lastFound {
		afterLastInRange++
	}
	...
	if tqp.excludeBefore(firstInRange) {
		if firstInRange < afterLastInRange {
			ips = append(ips, indexPositionBoundary{p: tqp.ProvenEntries[firstInRange].IndexPosition, d: 1})
		}
	} else {
	...
	for i := firstInRange + 1; i < afterLastInRange; i++ {
		if tqp.excludeBefore(i) {
			ips = append(ips,
				indexPositionBoundary{p: tqp.ProvenEntries[i-1].IndexPosition, d: -1},
				indexPositionBoundary{p: tqp.ProvenEntries[i].IndexPosition, d: 1})
		}
	}
	if tqp.excludeBefore(afterLastInRange) {
		if firstInRange < afterLastInRange {
			ips = append(ips, indexPositionBoundary{p: tqp.ProvenEntries[afterLastInRange-1].IndexPosition, d: -1})
		}
	} else {
...
func makeProofMap(storageProof []hexutil.Bytes) map[common.Hash][]byte {
