...
func (n *ethNode) assembleBlock(parentHash common.Hash, parentTimestamp uint64) (*engine.ExecutableDataV1, error) {
	...
	payloadAttribute := engine.PayloadAttributesV1{
		Timestamp:             timestamp,
		Random:                common.Hash{},
...
func (n *ethNode) insertBlock(eb engine.ExecutableDataV1) error {
	if !eth2types(n.typ) {
...
func (n *ethNode) insertBlockAndSetHead(parent *types.Header, ed engine.ExecutableDataV1) error {
	if !eth2types(n.typ) {
		return errors.New("invalid node type")
	}
