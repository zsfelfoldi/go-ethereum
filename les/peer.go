...
// Note: UpdateCapacity is a ClientPool callback and should not block.
...
func (p *clientPeer) announceOrStore(announce announceData) {
	...
	if p.capacity != 0 {
		p.announce(announce)
	} else if announce.Hash != p.lastAnnounce.Hash {
		p.lastAnnounce = announce
		p.lastAnnounceSent = false
	}
}

// announce sends the given head announcement to the client if it hasn't been sent before
func (p *clientPeer) announce(announce announceData) {
	if !p.lastAnnounceSent || announce.Hash != p.lastAnnounce.Hash {
		if !p.queueSend(func() { p.sendAnnounce(announce) }) {
			p.Log().Debug("Dropped announcement because queue is full", "number", announce.Number, "hash", announce.Hash)
		} else {
			p.Log().Debug("Sent announcement", "number", announce.Number, "hash", announce.Hash)
		}
		p.lastAnnounce = announce
		p.lastAnnounceSent = true
	}
}
