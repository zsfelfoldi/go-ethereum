...
func (pool *serverPool) selectPeer(canSend func(*peer) (bool, uint64)) *peer {
	...
	for _, entry := range pool.entries {
		if entry.state == psRegistered {
			...
			ok, cost := canSend(p)
			if ok {
				w := int64(1000000000 * math.Exp(-(entry.responseStats.recentAvg()+float64(cost))/float64(responseScoreTC)) * math.Pow((1-entry.timeoutStats.recentAvg()), timeoutPow))
...
func (pool *serverPool) loadNodes() {
	...
	for _, e := range list {
		glog.V(logger.Info).Infof("loaded node info %v  fails: %v  connStats: %v / %v  delayStats: %v / %v  responseStats: %v / %v  timeoutStats: %v / %v", e.id.String()+"@"+e.lastConnected.strKey(), e.lastConnected.fails, e.connectStats.avg, e.connectStats.weight, time.Duration(e.delayStats.avg), e.delayStats.weight, time.Duration(e.responseStats.avg), e.responseStats.weight, e.timeoutStats.avg, e.timeoutStats.weight)
		pool.entries[e.id] = e
