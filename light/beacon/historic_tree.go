...
func (ht *HistoricTree) addRoots(firstSlot uint64, blockRoots, stateRoots MerkleValues, deleteAfter bool, tailProof MultiProof) {
	...
	if firstSlot < ht.tailSlot {
		ht.tailSlot = firstSlot
	}
	...
	tailProofPeriod := (firstSlot + 1) >> 13 // - 1 // tail proof is not expected when firstSlot is in period 0
	fmt.Println("*** addRoots   tailProofPeriod", tailProofPeriod, "ht.tailPeriod", ht.tailPeriod, "ht.nextPeriod", ht.nextPeriod, "tailProof.Format != nil", tailProof.Format != nil)
	if (ht.nextPeriod == 0 || tailProofPeriod < ht.tailPeriod) && tailProof.Format != nil {
		if historicRootsRoot, ok := ht.HeadBlock.GetStateValue(BsiHistoricRoots); ok {
			...
			tailProofIndex := (0x2000000 + tailProofPeriod) * 2
