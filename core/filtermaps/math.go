...
func (p *Params) deriveFields() {
	...
	p.columnMask = (uint32(1) << p.logMapWidth) - 1
...
func (p *Params) potentialMatches(rows []FilterRow, mapIndex uint32, logValue common.Hash) potentialMatches {
	...
	for i, row := range rows {
		...
		for i := 0; i < rowLen; i++ {
			if potentialSubIndex := ((((((((row[i] * mul4) ^ xor2) * mul3) - sub2) * mul2) ^ xor1) * mul1) - sub1) & p.columnMask; potentialSubIndex < uint32(p.valuesPerMap) {
