...
var (
	...
	ppNodeInfoField = utils.NewNodeStateField("ppNodeInfo", reflect.TypeOf(&ppNodeInfo{}), nil, false, nil, nil)
	CapacityField   = utils.NewNodeStateField("capacity", reflect.TypeOf(uint64(0)), []*utils.NodeStateFlag{ActiveFlag}, false, nil, nil)
)
...
type PriorityPool struct {
	...
	stActive, stInactive            utils.NodeStateBitMask
...
func NewPriorityPool(ns *utils.NodeStateMachine, clock mclock.Clock, minCap uint64, minBias time.Duration, capacityStepDiv uint64, nodeField *utils.NodeStateField) *PriorityPool {
	pp := &PriorityPool{
		ns:               ns,
		clock:            clock,
	...
	ns.AddFieldSub(nodeFieldIndex, func(id enode.ID, state utils.NodeStateBitMask, oldValue, newValue interface{}) {
		if newValue != nil {
			c := &ppNodeInfo{
				id:            id,
			...
			ns.SetField(id, pp.ppNodeFieldIndex, c)
			if state&(pp.stActive|pp.stInactive) != 0 {
				pp.connectedNode(c)
			}
		} else {
			...
			ns.UpdateState(id, 0, pp.stActive|pp.stInactive, 0)
			ns.SetField(id, pp.ppNodeFieldIndex, nil)
		}
	})
	ns.AddStateSub(pp.stActive|pp.stInactive, func(id enode.ID, oldState, newState utils.NodeStateBitMask) {
		if c := pp.nodeInfo(id); c != nil {
			if oldState == 0 {
				pp.connectedNode(c)
			}
...
func activePriority(a interface{}, now mclock.AbsTime) int64 {
	...
	if c.bias == 0 {
		return invertPriority(c.node.Priority(now, c.capacity))
	} else {
		return invertPriority(c.node.EstMinPriority(now+mclock.AbsTime(c.bias), c.capacity))
	}
}
...
func activeMaxPriority(a interface{}, until mclock.AbsTime) int64 {
	...
	if c.bias == 0 {
		return invertPriority(c.node.EstMinPriority(until, c.capacity))
	} else {
		return invertPriority(c.node.EstMinPriority(until+mclock.AbsTime(c.bias), c.capacity))
	}
}

func (pp *PriorityPool) inactivePriority(p *ppNodeInfo) int64 {
	return p.node.EstMinPriority(pp.clock.Now()+mclock.AbsTime(pp.minBias), pp.minCap)
}

func (pp *PriorityPool) nodeInfo(id enode.ID) *ppNodeInfo {
	if c := pp.ns.GetField(id, pp.ppNodeFieldIndex); c != nil {
		return c.(*ppNodeInfo)
	}
	return nil
}
...
func (pp *PriorityPool) RequestCapacity(id enode.ID, targetCap uint64, bias time.Duration, setCap bool) (int64, bool) {
	...
	if targetCap > c.capacity {
		priority = c.node.EstMinPriority(pp.clock.Now()+mclock.AbsTime(bias), targetCap)
	} else {
		priority = c.node.Priority(pp.clock.Now(), targetCap)
	}
	pp.markForChange(c)
