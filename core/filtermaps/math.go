...
func (p *Params) maskedMapIndex(mapIndex, layerIndex uint32) uint32 {
	return mapIndex & (uint32(math.MaxUint32) << p.logMappingFrequency[min(layerIndex, uint32(len(p.logMappingFrequency))-1)])
}
...
func (p *Params) potentialMatches(rows []FilterRow, mapIndex uint32, logValue common.Hash) potentialMatches {
	...
	for i, row := range rows {
		rowLen, maxLen := len(row), int(p.maxRowLength[i])
		if rowLen > maxLen {
