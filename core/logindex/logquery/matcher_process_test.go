...
func TestMatcherProcess(t *testing.T) {
	for _, reverse := range []bool{false, true} {
		for _, limited := range []bool{false, true} {
			testMatcherProcess(t, reverse, limited)
		}
	}
}

func testMatcherProcess(t *testing.T, reverse, limited bool) {
	blockCount := uint64(1024)
	desc := fmt.Sprintf("[reverse %v, limited %v, minTopicCount %d]", reverse, limited)
	ts := newTestScheduler(t, desc, blockCount, blockCount, 64, reverse)
	...
	session := newSession(context.Background(), nil, common.Hash{}, maxResults, reverse)
...
func newTestScheduler(t *testing.T, desc string, blockCount, tableSize uint64, matchDensity int, reverse bool) *testScheduler {
	ts := &testScheduler{
		t:            t,
		desc:         desc,
