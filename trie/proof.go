...
// absence of the key.
func (t *Trie) Prove(key []byte, fromLevel uint, proofDb Database) error {
...
// wrong value.
func VerifyProof(rootHash common.Hash, key []byte, proofDb Database) (value []byte, err error, nodes int) {
	key = compactHexDecode(key)
	wantHash := rootHash[:]
	for i := 0; ; i++ {
