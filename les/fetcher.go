	...
	if n != nil {
		...
		for uint64(fp.nodeCnt)+head.Number-n.number > maxNodeCount && fp.root != nil {
			...
			for i, nn := range fp.root.children {
				if f.chain.GetStoredCanonicalHash(nn.number) == nn.hash {
...
func (f *lightFetcher) peerHasBlock(p *peer, hash common.Hash, number uint64) bool {
	...
	return f.chain.GetStoredCanonicalHash(fp.root.number) == fp.root.hash && f.chain.GetStoredCanonicalHash(number) == hash
}
...
func (f *lightFetcher) checkSyncedHeaders(p *peer) {
	...
	if n == nil {
		...
		if !f.checkAnnouncedHeaders(fp, []*types.Header{header}, []*big.Int{td}) {
			glog.V(logger.Debug).Infof("announce inconsistency after sync by peer %v", p.id)
			go f.pm.removePeer(p.id)
		}
	}
}
