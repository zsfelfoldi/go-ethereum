...
func New(stack *node.Node, config *ethconfig.Config) (*LightEthereum, error) {
	...
	leth.serverPool = newServerPool(ns, lespayDb, leth.valueTracker, time.Second, leth.prenegQuery, &mclock.System{}, config.UltraLightServers)
...
// LespayRequest sends a batch of requests to the given node through discv5 UDP TalkRequest and returns the responses
func (s *LightEthereum) LespayRequest(n *enode.Node, reqs lespay.Requests) lespay.Replies {
	reqsEnc, _ := rlp.EncodeToBytes(&reqs)
	repliesEnc, _ := s.p2pServer.DiscV5.TalkRequest(s.serverPool.dialNode(n), "lespay", reqsEnc)
...
// enrVersion returns the les and lespay UDP versions advertised in the ENR record
func (s *LightEthereum) enrVersion(n *enode.Node) (lesVersion, udpVersion uint) {
	if n.Seq() == 0 {
		var err error
		if n, err = s.p2pServer.DiscV5.RequestENR(n); n != nil && err == nil && n.Seq() != 0 {
			s.serverPool.ns.Persist(n)
		} else {
			return 0, 0
		}
	}
	...
	rlp.DecodeBytes(les[1], &udpVersion)
	return
}
...
func (s *LightEthereum) prenegQuery(n *enode.Node) int {
	...
	replies := s.LespayRequest(n, requests)
...
func (s *LightEthereum) Stop() error {
	close(s.closeCh)
	s.serverPool.stop()   // stop dialing new peers
	s.peers.close()       // stop accepting new connections
	s.valueTracker.Stop() // save peer stats
	s.reqDist.close()
	s.odr.Stop()
	s.relay.Stop()
	...
	s.pruner.close()
	s.eventMux.Stop()
	s.ns.Stop() // shut down peer logic after all other shutdown ops have finished
