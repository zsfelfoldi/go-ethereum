...
import (
	...
	lps "github.com/ethereum/go-ethereum/les/vflux/server"
...
func TestCapacityUpdateLes5(t *testing.T) {
	...
	req(0)
	exp(testMinCap, testMinCap, true)
	balance, _ := server.handler.server.ns.GetField(server.peer.cpeer.Node(), balanceTrackerSetup.BalanceField).(*vfs.NodeBalance)
