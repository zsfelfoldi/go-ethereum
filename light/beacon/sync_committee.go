...
func (s *SyncCommitteeTracker) deleteBestUpdate(period uint64) {
	...
	s.committeeRootCache.Remove(period)
	s.updateInfoChanged()
}
...
func (s *SyncCommitteeTracker) nextRequest(sp *sctPeerInfo) CommitteeRequest {
	...
	syncFirst, syncAfterLast := s.constraints.PeriodRange()
	...
	fmt.Println(" committee constraint range:", syncFirst, syncAfterLast)

	if syncAfterLast < syncFirst+2 {
		return CommitteeRequest{}
	}
	syncFirst++
	...
	fmt.Println(" sync range:", syncFirst, syncAfterLast)

	var (
		request  CommitteeRequest
		reqCount int
	)
	if localInfo.AfterLastPeriod == 0 {
		// no local committee chain yet
		localFirstPeriod, localInfo.AfterLastPeriod = syncAfterLast, syncAfterLast
		request.CommitteePeriods = []uint64{syncAfterLast - 1, syncAfterLast}
		reqCount = CommitteeCostFactor * 2
	}

	sharedFirst, sharedAfterLast := syncFirst, syncAfterLast
	if localFirstPeriod > sharedFirst {
		sharedFirst = localFirstPeriod
	}
	if localInfo.AfterLastPeriod < sharedAfterLast {
		sharedAfterLast = localInfo.AfterLastPeriod
	}
	if sp.forkPeriod < sharedAfterLast {
		sharedAfterLast = sp.forkPeriod
	}
	if sharedFirst > sharedAfterLast {
		return CommitteeRequest{}
	}
	fmt.Println(" shared range:", sharedFirst, sharedAfterLast)

	for period := sharedFirst; period < sharedAfterLast && reqCount < MaxCommitteeUpdateFetch; period++ {
	...
	for period := sharedAfterLast; period < syncAfterLast && reqCount+CommitteeCostFactor < MaxCommitteeUpdateFetch; period++ {
		request.UpdatePeriods = append(request.UpdatePeriods, period)
		request.CommitteePeriods = append(request.CommitteePeriods, period+1)
		reqCount += CommitteeCostFactor + 1
	}
	for period := sharedFirst - 1; period >= syncFirst && reqCount+CommitteeCostFactor < MaxCommitteeUpdateFetch; period-- {
		request.UpdatePeriods = append(request.UpdatePeriods, period)
		request.CommitteePeriods = append(request.CommitteePeriods, period-1)
		reqCount += CommitteeCostFactor + 1
	}
...
type sctConstraints interface {
	PeriodRange() (first, afterLast uint64)
	//CommitteeRootMatch(period uint64, root common.Hash) bool // nothing matches outside PeriodRange, some periods in the range might match a single root only or any root
			...
			for {
				...
				case <-wsc.initTriggerCh:
					...
					if block, err := backend.GetInitBlock(ctx, checkpoint); err == nil {
						...
						var ok bool
						if initData, ok = block.getInitData(); ok { // error log is printed if not
							break loop
						}
					}
				}
...
func (wsc *WeakSubjectivityCheckpoint) PeriodRange() (first, afterLast uint64) {
	wsc.lock.RLock()
	...
	if wsc.parent != nil {
		first, afterLast = wsc.parent.PeriodRange()
	}
	if afterLast < wsc.initData.Period {
