...
func (mp *matcherProcess) run() {
	...
	for !mp.finished {
		...
		if pos == nil {
			...
			//fmt.Println("mp.addMatch(pos)", pos)
		...
		//fmt.Println("len(mp.allMatches)", len(mp.allMatches), "mp.session.maxResults", mp.session.maxResults)
		if suspendNow || cumulativeResults+uint64(len(mp.allMatches)) >= uint64(mp.session.maxResults) {
			//fmt.Println(mp.firstBlock, mp.lastBlock, "*** return suspendNow", suspendNow, "cumulativeResults", cumulativeResults, "len(mp.allMatches)", len(mp.allMatches), "mp.session.maxResults", mp.session.maxResults)
			return
		}
	}
	//fmt.Println(mp.firstBlock, mp.lastBlock, "*** return finished", mp.finished, "len(mp.allMatches)", len(mp.allMatches), "len(mp.allMatches)", len(mp.allMatches))
}
...
func (mp *matcherProcess) updateEstimatedResults() (uint64, bool) {
	if mp.finished {
		...
		atomic.StoreUint64(&mp.estimatedResults, uint64(len(mp.allMatches)))
		return uint64(len(mp.allMatches)), false
	}
	var (
	...
	if len(mp.positions) > 0 {
		...
		remainingResults := uint64(float64(len(mp.allMatches)) * ratio)
		estimatedResults = uint64(len(mp.allMatches)) + remainingResults
