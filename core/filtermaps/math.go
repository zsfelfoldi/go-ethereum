...
// addressValue returns the map value hash of a log emitting address.
...
// topicValue returns the map value hash of a log topic.
...
// txValue returns the map value hash of a transaction entry.
func txValue(txHash common.Hash) common.Hash {
	var result common.Hash
	hasher := sha256.New()
	hasher.Write(txHash[:])
	suffix := [1]byte{1}
...
// blockValue returns the map value hash of a block entry.
func blockValue(blockHash common.Hash) common.Hash {
	var result common.Hash
	hasher := sha256.New()
...
func (p *Params) indexEntryNodeWeight(bytesUsed uint32) float32 {
...
func (p *Params) sanitize() error {
	...
	if weight := p.indexEntryNodeWeight(0) * float32(p.valuesPerMap); p.storageLevel(weight*0.99) < 2 {
...
// higher order layers.
func (p *Params) rowIndex(mapIndex, layerIndex uint32, mapValue common.Hash) uint32 {
	hasher := sha256.New()
	hasher.Write(mapValue[:])
	var indexEnc [8]byte
...
func (p *Params) columnIndex(lvIndex uint64, mapValue *common.Hash) uint32 {
	var indexEnc [8]byte
...
// here.
func (p *Params) potentialMatches(rows []FilterRow, mapIndex uint32, mapValue common.Hash) potentialMatches {
	...
	for i, row := range rows {
		...
		for i := 0; i < rowLen; i++ {
			if potentialMatch := mapFirst + uint64(row[i]>>(p.logMapWidth-p.logValuesPerMap)); row[i] == p.columnIndex(potentialMatch, &mapValue) {
