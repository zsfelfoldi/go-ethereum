...
func TestTrieForkGC(t *testing.T) {
	...
	for i := 0; i < triesInMemory; i++ {
		chain.stateCache.TrieDB().Dereference(blocks[len(blocks)-1-i].Root(), common.Hash{}, nil)
		chain.stateCache.TrieDB().Dereference(forks[len(blocks)-1-i].Root(), common.Hash{}, nil)
	}
