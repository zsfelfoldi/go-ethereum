...
type indexServer struct {
	...
	readPriorityCh                  chan int // Indexer => callback -> historicReadLoop (cap=1)
...
func (s *indexServer) setMaxPriority(priority int) {
	...
	s.deliverPriorityCh <- priority

	select {
	case <-s.readPriorityCh:
	default:
	}
	s.readPriorityCh <- priority
}
...
func (s *indexServer) historicReadLoop() {
	...
	for {
		...
		switch belowPriority {
		case 0:
			select {
			case <-s.parent.closeCh:
				return
			case maxPriority = <-s.readPriorityCh:
