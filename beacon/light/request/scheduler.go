...
func (s *Scheduler) handleEvent(server Server, event Event) {
	s.Trigger()
	switch event.Type {
	case EvResponse, EvFail, EvTimeout:
		var idr IdAndResponse
		if event.Type == EvResponse {
			idr = event.Data.(IdAndResponse)
		} else {
			idr.ID = event.Data.(ID)
		}
		id := ServerAndId{Server: server, Id: idr.ID}
		if pr, ok := s.pending[id]; ok {
			tracker := s.trackers[pr.module]
			tracker.requestEvents = append(tracker.requestEvents, RequestEvent{
				ServerAndId: id,
				Request:     pr.request,
				Response:    idr.Response,
				Timeout:     event.Type == EvTimeout || pr.timeout,
				Finalized:   event.Type != EvTimeout,
			})
		...
		return // do not add to server events
	case EvUnregistered:
