...
func getLogFromReceipts(receipts types.Receipts, relativePos uint64) (*types.Log, bool) {
	for _, receipt := range receipts {
		for _, log := range receipt.Logs {
			if relativePos == 0 {
				return log, true
			}
			if sub := uint64(len(log.Topics) + 1); relativePos >= sub {
				relativePos -= sub
			} else {
				return nil, true // log position is in block range but there is no log there (false positive)
			}
		}
	}
	return nil, false // invalid log position
}
...
func (f *Filter) indexedLogs(ctx context.Context, chainView *filtermaps.ChainView, indexView *filtermaps.IndexView, begin, end uint64) ([]*types.Log, error) {
	...
	for _, logPosition := range potentialMatches {
		...
		log, ok := getLogFromReceipts(receipts, logPosition.RelativePos)
		if !ok { // position not found
			err = fmt.Errorf("invalid log position: block #%d relative position %d", logPosition.BlockNumber, logPosition.RelativePos)
			break
		}
		if log != nil {
